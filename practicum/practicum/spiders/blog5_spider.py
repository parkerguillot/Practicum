...
class Blog5SpiderSpider(scrapy.Spider):
    ...
    def parse_pages(self, response):
        ...
        article_date = response.xpath('//p[@class = "post-meta"]//text()').re(r"\w+\s\d{1,2},\s\d{4}")
        article_title = response.xpath('//h1//text()').extract()
        author = response.xpath('//p[@class = "post-author"]//text()').re(r"[A-Z]\w+\s\w+")
